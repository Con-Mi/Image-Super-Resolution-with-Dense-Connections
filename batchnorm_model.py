...
class _DenseLayer(nn.Sequential):
	def __init__(self, num_input_features, growth_rate, bot_neck, drop_rate):
		...
		self.add_module('leaky_relu1', nn.SELU(inplace = True)),
		...
		self.add_module('leaky_relu2', nn.SELU(inplace = True)),
...
class _TransitionLayer(nn.Sequential):
	def __init__(self, num_input_features, num_output_features):
		...
		self.add_module('leaky_relu', nn.SELU(inplace  = True))
...
class SRDenseNetwork(nn.Module):
	def __init__(self, growth_rate = 32, block_config = (6, 12, 24, 16), num_init_features = 64, bot_neck = 4, drop_rate = 0, upscale_factor = 2):
		...
		self.features = nn.Sequential(OrderedDict([
			...
			('leaky_relu1', nn.SELU(inplace = True)),
		...
		self.features.add_module('conv_bottleneck', nn.Conv2d(in_channels = num_features, out_channels = 3*upscale_factor**2, kernel_size = 1, stride = 1, bias = False))
	...
	def forward(self, x):
		...
		out = F.selu(features, inplace = True)
